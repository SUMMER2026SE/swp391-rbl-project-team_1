    ...
    io.on("connection", (socket) => {
        ...
        socket.on("send-direct-message", async ({ conversationId, message }) => {
            try {
                ...
                // Update in DB
                const updatedMessage = await client_1.default.message.update({
                    where: { id: message.id },
                    data: { status, isRead }
                });
        ...
        socket.on("video_call_invite", async (data) => {
            ...
            try {
                ...
                const callLog = await client_1.default.videoCallLog.create({
                    data: {
        ...
        socket.on("video_call_accepted", async (data) => {
            ...
            try {
                if (data.callId) {
                    await client_1.default.videoCallLog.update({
        ...
        socket.on("end-call", async (data) => {
            ...
            try {
                if (data.callId) {
                    const log = await client_1.default.videoCallLog.findUnique({
                        where: { id: data.callId }
                    });
                    if (log) {
