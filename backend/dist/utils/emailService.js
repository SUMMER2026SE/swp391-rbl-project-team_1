...
exports.sendBookingNotificationToDoctorEmail = sendBookingNotificationToDoctorEmail;
 ...
 */
async function sendResetPasswordOtpEmail(email, otp, recipientName) {
    if (!process.env.MAIL_USER || !process.env.MAIL_PASSWORD) {
 ...
 */
async function sendBookingConfirmationEmail(email, details) {
    if (!process.env.MAIL_USER || !process.env.MAIL_PASSWORD)
        return;
    try {
 ...
 */
async function sendBookingStatusUpdateEmail(email, details) {
    if (!process.env.MAIL_USER || !process.env.MAIL_PASSWORD)
        return;
    try {
        const dateStr = details.appointmentDate.toLocaleDateString("vi-VN", {
 ...
 */
async function sendBookingReminderEmail(email, details) {
    if (!process.env.MAIL_USER || !process.env.MAIL_PASSWORD)
        return;
    try {
        const dateStr = details.appointmentDate.toLocaleDateString("vi-VN", {
 ...
 */
async function sendBookingNotificationToDoctorEmail(email, details) {
    if (!process.env.MAIL_USER || !process.env.MAIL_PASSWORD)
        return;
    try {
        const dateStr = details.appointmentDate.toLocaleDateString("vi-VN", {
 ...
 */
async function sendAbsenceEmail(email, details) {
    if (!process.env.MAIL_USER || !process.env.MAIL_PASSWORD)
        return;
    try {
        const dateStr = details.appointmentDate.toLocaleDateString("vi-VN", {
...
function initReminderScheduler() {
    ...
    checkAndSendReminders().catch((err) => console.error("[Scheduler] Initial startup reminder check failed:", err));
    checkAndProcessAbsences().catch((err) => console.error("[Scheduler] Initial startup absence check failed:", err));
    // Then run reminder every hour (3600000 milliseconds)
    setInterval(() => {
        checkAndSendReminders().catch((err) => console.error("[Scheduler] Scheduled reminder check failed:", err));
    }, 60 * 60 * 1000);
    // And run absence check every 30 minutes (1800000 milliseconds)
    setInterval(() => {
        checkAndProcessAbsences().catch((err) => console.error("[Scheduler] Scheduled absence check failed:", err));
    }, 30 * 60 * 1000);
}
/**
 ...
 */
async function checkAndProcessAbsences() {
    try {
        const now = new Date();
        ...
        const missedAppointments = await client_1.default.appointment.findMany({
            where: {
                status: "CONFIRMED",
 ...
 */
async function sendPrescriptionEmail(email, details, pdfBase64) {
    if (!process.env.MAIL_USER || !process.env.MAIL_PASSWORD)
        return;
    try {
        const dateStr = details.appointmentDate.toLocaleDateString("vi-VN");
 ...
 */
async function sendCertificateVerificationEmail(email, doctorName, certTitle, status, reason) {
    if (!process.env.MAIL_USER || !process.env.MAIL_PASSWORD) {
