...
async function callGeminiApi(prompt, history) {
    ...
    if (!response.ok) {
        ...
        throw new apiError_1.ApiError(`Gemini API returned error: ${response.statusText}`, response.status);
    }
 ...
 */
function getFallbackMedicalResponse(userQuery) {
 ...
 */
async function getMedicalDiagnosis(userMessage, chatHistory) {
    try {
        if (!userMessage || userMessage.trim() === "") {
            throw new apiError_1.ApiError("User message cannot be empty", 400);
        }
        // Clean user message
