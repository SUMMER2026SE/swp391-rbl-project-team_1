    ...
    try {
      ...
      `,
        ...
        if (process.env.NODE_ENV === "development") {
            console.log("=========================================");
            console.log(`[DEVELOPMENT MODE] Bypassing email failure.`);
            console.log(`MÃ OTP CỦA BẠN LÀ: ${otp}`);
            console.log("=========================================");
            // Don't throw error in development so flow can continue
            return;
        }
...
async function sendResetPasswordOtpEmail(email, otp, recipientName) {
    ...
    try {
      ...
      `,
        ...
        if (process.env.NODE_ENV === "development") {
            console.log("=========================================");
            console.log(`[DEVELOPMENT MODE] Bypassing reset email failure.`);
            console.log(`MÃ OTP KHÔI PHỤC CỦA BẠN LÀ: ${otp}`);
            console.log("=========================================");
            return;
        }
...
function initReminderScheduler() {
    ...
    checkAndSendReminders().catch((err) => console.error("[Scheduler] Initial startup check failed:", err));
    // Then run it every hour (3600000 milliseconds)
    setInterval(() => {
        checkAndSendReminders().catch((err) => console.error("[Scheduler] Scheduled check failed:", err));
    }, 60 * 60 * 1000);
}
