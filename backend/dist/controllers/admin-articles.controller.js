...
async function createArticleHandler(req, res, next) {
    try {
        const { title, content, thumbnail, published, type } = req.body;
        if (!title || !content) {
            throw new apiError_1.ApiError("Title and content are required", 400);
        }
        const authorId = req.user?.userId;
        const article = await (0, admin_articles_service_1.createArticle)({ title, content, thumbnail, published, type, authorId });
        res.status(201).json({
...
async function updateArticleHandler(req, res, next) {
    try {
        ...
        const { title, content, thumbnail, published, type } = req.body;
        const article = await (0, admin_articles_service_1.updateArticle)(id, { title, content, thumbnail, published, type });
        res.json({
