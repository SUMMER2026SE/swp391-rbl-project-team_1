import { PrismaClient, AppointmentStatus, Role, ComplaintType, ComplaintStatus } from '@prisma/client';
...
async function main() {
    ...
    const users = await prisma.user.findMany({ where: { role: Role.USER }, take: 10 });
    ...
    for (let i = 0; i < 300; i++) {
        ...
        const status: AppointmentStatus = isCompleted ? 'COMPLETED' : 'CANCELLED';
        ...
        newAppointments.push({
            ...
            patientProfileType: 'SELF' as any,
        });
    }
    ...
    for (let i = 0; i < 5; i++) {
        ...
        newComplaints.push({
            ...
            status: i % 2 === 0 ? ComplaintStatus.PENDING : ComplaintStatus.RESOLVED,
    ...
    for (const appt of recentAppointments) {
        ...
        newComplaints.push({
            ...
            status: Math.random() > 0.5 ? ComplaintStatus.PENDING : ComplaintStatus.RESOLVED,
