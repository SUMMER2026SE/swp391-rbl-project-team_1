...
export const getDashboardCharts = async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        const barChartMap = new Map<string, { name: string, completed: number, confirmed: number, cancelled: number }>();
        for (let i = 6; i >= 0; i--) {
        ...
        last7DaysAppointments.forEach(app => {
            ...
            if (barChartMap.has(dateStr)) {
                ...
                if (app.status === 'COMPLETED') data.completed++;
                else if (app.status === 'CONFIRMED') data.confirmed++;
                else if (app.status === 'CANCELLED') data.cancelled++;
            }
        });
        ...
        const lineChartMap = new Map<string, Set<string>>(); // month-year -> set of userIds
        for (let i = 2; i >= 0; i--) {
...
export const updateBulkAppointmentStatus = async (req: AuthenticatedRequest, res: Response) => {
    try {
        const doctor = await getDoctor(req.user!.userId);
        if (!doctor) return res.status(404).json({ message: "Doctor profile not found" });
...
export const getDoctorStatistics = async (req: AuthenticatedRequest, res: Response) => {
    try {
        const doctor = await getDoctor(req.user!.userId);
        if (!doctor) return res.status(404).json({ message: "Doctor profile not found" });
        ...
        patientMap.forEach(count => { if (count > 1) returnPatients++; });
        ...
        const revenue12Months: any[] = [];
        const patients12Months: any[] = [];
        for (let i = 11; i >= 0; i--) {
            const d = new Date(today.getFullYear(), today.getMonth() - i, 1);
            const monthStr = `T${d.getMonth() + 1}/${d.getFullYear().toString().slice(2)}`;
        ...
        const icdMap = new Map<string, number>();
        medicalRecords.forEach(r => {
            const code = r.icd10Code?.trim();
            if (code) icdMap.set(code, (icdMap.get(code) || 0) + 1);
        });
