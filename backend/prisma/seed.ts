...
async function main() {
  ...
  const webDev = await prisma.skill.create({ data: { name: 'Web Development', slug: 'web-dev' } });
  const dataSci = await prisma.skill.create({ data: { name: 'Data Science', slug: 'data-science' } });
  const mobileDev = await prisma.skill.create({ data: { name: 'Mobile Development', slug: 'mobile-dev' } });
  ...
  const subSkills = [];
  for (const s of [...webSkills, ...dsSkills, ...mobSkills]) {
    const created = await prisma.skill.create({ data: s });
    subSkills.push(created);
  }

  const allSkills = [webDev, dataSci, mobileDev, ...subSkills];
