  ...
  return (
    <aside className="w-64 bg-slate-950 border-r border-slate-900 h-screen sticky top-0 flex flex-col justify-between p-4 z-40">
      ...
      <div className="flex flex-col gap-4 border-t border-slate-900 pt-4">
        {user && (
          <Link href="/student/profile" className="flex items-center gap-3 px-3 py-1 hover:bg-slate-900/50 rounded-xl transition-colors cursor-pointer group">
            <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-indigo-500 to-pink-500 flex items-center justify-center text-white font-bold text-sm shadow-inner uppercase group-hover:shadow-md transition-all">
              {user.fullName.substring(0, 2)}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-slate-200 text-sm font-semibold truncate leading-none mb-1 group-hover:text-white transition-colors">
                {user.fullName}
              </p>
              <p className="text-slate-500 text-xs truncate leading-none group-hover:text-slate-400 transition-colors">
                {user.email}
              </p>
            </div>
          </Link>
