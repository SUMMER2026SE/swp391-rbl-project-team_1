...
export function Header() {
  ...
  useEffect(() => {
    const handleRealtimeNotification = (data: any) => {
      const newAlert: Alert = {
        id: data.alertId || Math.random().toString(),
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const response = await api.get('/risk/history'); // using risk history or notifications endpoint
      // We can query alerts through active user state or fallback to risk logs
    } catch (_) {}
  };
            ...
            {showDropdown && (
              <div className="absolute right-0 mt-3 w-80 bg-slate-900 border border-slate-800 rounded-2xl shadow-xl shadow-slate-950/50 overflow-hidden z-50">
                <div className="px-4 py-3 border-b border-slate-800 flex items-center justify-between bg-slate-950/50">
                  <span className="text-slate-200 font-bold text-sm">Cảnh báo học tập</span>
                ...
                <div className="max-h-72 overflow-y-auto divide-y divide-slate-800/50">
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((notif) => (
                      <div key={notif.id} className="p-4 hover:bg-slate-800/30 transition-colors flex gap-3 relative group">
                        <div className="flex-1">
                          <p className="text-xs font-semibold text-rose-400 mb-1">
                            {notif.type === 'RED_FLAG' ? '🚨 Cảnh báo đỏ' : '⚠️ Cảnh báo vàng'}
                          </p>
                        ...
                        <button
                          ...
                          className="text-slate-500 hover:text-slate-300 transition-colors p-1 self-start rounded"
                        >
