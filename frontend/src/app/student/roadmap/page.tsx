...
export default function RoadmapPage() {
  ...
  const handleExportPDF = async () => {
    try {
      ...
      if (!element) throw new Error('Không tìm thấy nội dung');

      // Hide action buttons temporarily for cleaner PDF
      const actionRow = document.getElementById('roadmap-action-row');
      ...
      pdf.save('EduPath_Roadmap.pdf');
      
      toast.success('Xuất PDF thành công! 🎉', { id: 'pdf-export' });
    } catch (error) {
      toast.error('Lỗi khi xuất PDF', { id: 'pdf-export' });
    }
  };
  ...
  return (
    <div id="roadmap-content" className="space-y-6 max-w-4xl mx-auto font-sans pb-10 bg-slate-950 min-h-screen">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 relative select-none">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          {/* Action Row */}
          <div id="roadmap-action-row" className="flex flex-wrap items-center gap-3">
            <button
