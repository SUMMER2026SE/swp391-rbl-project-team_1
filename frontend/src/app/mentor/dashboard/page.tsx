...
export default function MentorDashboard() {
  ...
  useEffect(() => {
    const handleRedFlagAlert = (data: any) => {
      ...
      toast(`🚨 Cảnh báo mới: Học viên ${data.studentName} có điểm rủi ro ${data.riskScore}%!`, {
        icon: '⚠️',
        duration: 5000,
        style: {
          background: '#991B1B',
    ...
    try {
      ...
      if (redFlagsRes.data.success && redFlagsRes.data.redFlags) {
        const loadedAlerts: SocketAlert[] = redFlagsRes.data.redFlags.map((rf: any) => ({
          ...
          message: `⚠️ Sinh viên ${rf.fullName} đang ở trạng thái nguy cơ học tập cao (${rf.riskScore}%)`,
      ...
      {alerts.length > 0 && (
        <div className="bg-rose-950/10 border border-rose-900/30 rounded-3xl p-5 space-y-3">
          <div className="flex items-center gap-2 text-rose-400 font-bold text-sm">
            ...
            <span>DANH SÁCH CẢNH BÁO ĐỎ TỨC THÌ (REALTIME RED FLAGS)</span>
          </div>
                  ...
                  <div className="space-y-2 text-left">
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between text-[10px] text-slate-500">
                        <span>Đánh giá an toàn (Safety Score)</span>
